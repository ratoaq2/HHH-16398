...
public class ORMUnitTestCase extends BaseCoreFunctionalTestCase {
	...
	protected Class<?>[] getAnnotatedClasses() {
		return new Class<?>[] { EntityA.class, AbstractEntityB.class, EntityBChildOne.class, EntityBChildTwo.class };
	}
	...
	@Test
	public void hhh16398Test() throws Exception {
		...
		try (Session s = openSession()) {
			...
			EntityBChildOne childOne = new EntityBChildOne();
			childOne.setAction(EntityBChildOne.ChildOneAction.ACTION_1);
			childOne.entityA = entityA1;
			entityA1.entities.add(childOne);

			EntityA entityA2 = new EntityA();
			EntityBChildTwo childTwo = new EntityBChildTwo();
			childTwo.setAction(EntityBChildTwo.ChildTwoAction.ACTION_20);
			childTwo.entityA = entityA2;
			entityA2.entities.add(childTwo);
		...
		try (Session s = openSession()) {
			...
			Query<EntityA> query = s.createQuery("select c from EntityA c", EntityA.class);
			List<EntityA> actual = query.list();

			assertThat(actual).hasSize(2);
			assertThat(actual.get(0).entities).hasSize(1);
			assertThat(actual.get(1).entities).hasSize(1);
		}
	}
}
